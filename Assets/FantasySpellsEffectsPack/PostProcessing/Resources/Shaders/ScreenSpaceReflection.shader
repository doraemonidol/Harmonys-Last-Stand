...
Shader "Hidden/Post FX/Screen Space Reflection"
{
    ...
    float4 fragComposite(v2f i) : SV_Target
    {
        ...
        float3 ssrResult = UNITY_BRDF_PBS(0, specColor, oneMinusReflectivity, roughness, wsNormal, -eyeVec, light, ind).
            rgb * _SSRMultiplier;
    ...
    float4 fragCompositeSSR(v2f i) : SV_Target
    {
...

        {
            ...
            {
                result = lerp(bilateralUpsampleReflection(tsP, mipMin), bilateralUpsampleReflection(tsP, mipMax),
                    mipLerp);
            }
            else
            {
