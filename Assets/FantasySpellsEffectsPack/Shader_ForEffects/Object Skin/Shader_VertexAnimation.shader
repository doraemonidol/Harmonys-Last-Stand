Shader "GAPH Custom Shader/Object Skin/Vertex Animation"
{
    ...
    Category
    {
        Tags
        {
            "Queue" = "Transparent" "RenderType" = "Tranparent" "IgnoreProjector" = "True"
        }
        ...
        SubShader
        {
            Pass
            {
                ...
                v2f vert(appdata_t v)
                {
                    ...
                    float4 NoiseTex = tex2Dlod(
                        _NoiseMap, Noise + float4(float3(_Time.x / 2, _Time.y / 2, _Time.z * 2) * _NoiseScale * 10, 0));
                    v.vertex.xyz +=
