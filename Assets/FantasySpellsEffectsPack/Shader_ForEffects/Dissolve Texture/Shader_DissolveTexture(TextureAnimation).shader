    ...
    Category
    {
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                {
                    half2 distort = UnpackNormal(tex2D(_NormalTex, i.texcoord2 * 1.0f + (_Speed * _Time.x / 20))).rg;
                    // Get distort info from normal texture.
                    distort += UnpackNormal(
                        tex2D(_NormalTex, i.texcoord2 * 1.0f - (_Speed * _Time.x / 20) + float2(0.5f, 0.15f))).rg;

                    half4 tex = tex2D(_MainTex, i.texcoord.xy / 5 + distort.x * _DistortFactor - (_Speed * _Time / 20));
                    ...
                    tex2 *= tex2D(
                        _MainTex,
                        i.texcoord.xy + distort.xy * _DistortFactor - (_Speed * _Time / 10) + float2(0.15f, -0.15f));
                    tex2 *= tex2D(
                            _MainTex,
                            i.texcoord.xy + distort.xy * _DistortFactor - (_Speed * _Time / 10) + float2(-0.5f, 0.5f)) *
                        2.5f;
                    ...
                    half4 res = _ColorStrength * i.color * _TintColor * tex_all;
                    //Compose main r,g,b source using animate tex
