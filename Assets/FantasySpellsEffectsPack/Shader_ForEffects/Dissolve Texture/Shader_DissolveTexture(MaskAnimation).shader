    ...
    Category
    {
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                {
                    half2 distort = UnpackNormal(
                        tex2D(_NormalTex, i.texcoord2 * 1.0f + (_Speed * _Time.x / 20 * _DistortTimeFactor))).rg;
                    // Get distort info from normal texture.
                    distort += UnpackNormal(tex2D(
                        _NormalTex,
                        i.texcoord2 * 1.0f - (_Speed * _Time.x / 20 * _DistortTimeFactor) + float2(0.5f, 0.15f))).rg;

                    half4 tex = tex2D(_MainTex, i.texcoord.xy + distort.x * _DistortFactor - (_Speed * _Time / 20));
