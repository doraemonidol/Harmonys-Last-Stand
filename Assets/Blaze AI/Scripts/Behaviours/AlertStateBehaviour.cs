...
{
    [AddComponentMenu("Blaze AI/Alert State Behaviour")]
    public class AlertStateBehaviour : BlazeBehaviour
    {
        #region PROPERTIES
        ...
        [Tooltip(
            "Add animations for idle. One will be chosen at random. If only one is added then only that will play.")]
        public string[] idleAnim;

        [Tooltip("Movement animation.")] public string moveAnim;
        ...
        [Min(0)]
        [Tooltip(
            "Time in seconds to stay in idle before going to the next waypoint. Will generate a random number between the two values. For a fixed value make both inputs the same.")]
        public Vector2 idleTime = new(5f, 5f);

        [Tooltip("Will tell the Audio Manager component to play patrol audios in this state.")]
        public bool playPatrolAudio;

        [Tooltip(
            "Make the AI return to normal state after a specific amount of time. If disabled, the AI will never go back to normal state once it's out of it.")]
        ...
        [Tooltip(
            "If true will play an audio when returning to normal. Set the audios in the audio scriptable in the General tab in blaze AI.")]
        ...
        public override void Main()
        {
            ...
            if (movedToLocation && isIdle) ForceMove();
            ...
            if (avoidFacingObstacles) ObstacleRay();
        }

        private void OnDrawGizmosSelected()
        {
            if (showObstacleRay)
                Debug.DrawRay(transform.position + obstacleRayOffset,
                    transform.TransformDirection(Vector3.forward) * obstacleRayDistance, Color.yellow);
        }
        ...
        public virtual void PreSetWaypointsMove()
        {
            ...
            if (blaze.MoveTo(waypoint, moveSpeed, turnSpeed, moveAnim, animT))
            {
                ...
                if (blaze.WayPointTurning()) StartCoroutine("Idle");
            ...
            if (!blaze.isPathReachable) blaze.NextWayPoint();
        }
        ...
        public virtual void RandomizedWaypointsMove()
        {
            ...
            if (!blaze.isPathReachable) SetEndDestination();
        }
        ...
        public virtual IEnumerator Idle()
        {
            ...
            {
                var animIndex = Random.Range(0, idleAnim.Length);
            ...
            // set the wait time
            var _idleTime = Random.Range(idleTime.x, idleTime.y);
            SetEndDestination();
        ...
        private void ReturnToNormalTimer()
        {
            ...
            if (!isReturningToNormal) StartCoroutine("ReturnToNormal");
        }
        ...
        private void ResetReturnToNormal()
        {
            isReturningToNormal = false;
            returnToNormalTimer = 0;
        }
        ...
        {
            var distance = (waypoint - transform.position).sqrMagnitude;
            var minDistance = obstacleRayDistance * obstacleRayDistance;

            if (distance <= minDistance)
            {
                // AI should be facing the waypoint
                if (Vector3.Dot((waypoint - transform.position).normalized, transform.forward) < 0.8f) return;

                RaycastHit hit;
                if (Physics.Raycast(transform.position + obstacleRayOffset,
                        transform.TransformDirection(Vector3.forward), out hit, obstacleRayDistance, obstacleLayers))
                {
                    isIdle = true;
                    StartCoroutine("Idle");
                }
