    ...
    public class AttackStateBehaviour : BlazeBehaviour
    {
        ...
        public virtual void CallOthers()
        {
            ...
            for (var i = 0; i < callOthersNum; i += 1)
            {
                ...
                if (callOthersHitArr[i].transform.IsChildOf(lastEnemy.transform)) continue;
        ...
        [Tooltip(
            "If enabled, will adapt the distancing calculations to suit that of a ranged AI. So for example: if the target is unreachable but within attack distance then the AI will attack.")]
        public bool ranged;

        [Min(0)]
        [Tooltip(
            "The idle distance between the AI and the target waiting to attack. If distance between the AI and the target is more than this value, the AI will chase the player to close the distance.")]
        ...
        [Tooltip(
            "Add your attacks. One will be chosen at random on each attack. If only one is set then that one will be launched every time.")]
        ...
        [Min(0)]
        [Tooltip(
            "The amount of time to pass before attacking. A value will be randomized between the two inputs. For a constant value set the two inputs to the same value.")]
        public Vector2 attackInIntervalsTime = new(1, 4);
        ...
        [Tooltip(
            "If enabled, will turn to face the target if need be when waiting to attack. Will use the turn speed found in blaze AI > Waypoints > Turn Speed.")]
        public bool turnToTarget = true;

        [Tooltip(
            "If dot product between the AI and it's target is less or equal to this value then turning will occur.")]
        [Range(0, 1)]
        public float turnSensitivity = 0.7f;
        ...
        [Min(-1)]
        [Tooltip(
            "The amount of time to strafe for. A value will be randomized between the two inputs. For a constant value, set both inputs to the same value. For infinity and never stop strafing set both values to -1.")]
        public Vector2 strafeTime = new(3, 5);

        [Min(0)] [Tooltip("The amount of time to wait before strafing again.")]
        public Vector2 strafeWaitTime = new(0.3f, 1);
        ...
        [Min(0)]
        [Tooltip(
            "Keep rotating/facing the enemy while attacking. So if the AI throws a punch it'll still keep rotating to it's target while the punch animation is playing. Not good for melee but good for ranged enemies when you always want them to be looking at the target.")]
        ...
        [Tooltip(
            "Play a random audio from the audio scriptable when AI is waiting for it's turn to attack target. Set the audios from the audio scriptable in blaze AI > General tab.")]
        public bool playAttackIdleAudio;

        [Min(0)]
        [Tooltip(
            "The amount of time to pass (seconds) to play audio. A random number will be generated between the two set values. For a constant time, set the two properties to the same value.")]
        public Vector2 attackIdleAudioTime = new(3, 10);
        ...
        [Tooltip(
            "Play a random audio when AI is moving to attack. Set audio from audio scriptable in the Move To Attack array.")]
        public bool playAudioOnMoveToAttack;

        [Tooltip(
            "If set to true, on moving to attack the AI will always play an audio on chase. If false, there will be a 50/50 chance whether a chase audio will be played or not.")]
        ...
        private readonly int checkPathFrames = 3;
        ...
        private bool startAttackInIntervals;
        private bool isStrafing;
        private bool isStrafeWait;
        private bool turningToTarget;
        private bool returnedToAlert;
        private bool isSearching;
        private bool returnPatrolAudioPlayed;
        private bool isReachable;
        private bool isReachableCantHit;
        private bool attackIdleAudioPlayed;
        private bool shouldPlayChaseAudio;
        private bool chaseAudioPlayed;
        private bool chaseAudioIsReady;
        private bool moveToAttackAudioPlayed;
        private bool isAddedToManagerInAttacks;
        ...
        private float _intervalAttackTime;
        ...
        private float attackIdleAudTimer;
        private float chosenAttackIdleAudTime = -1;
        private readonly float collSize = 0f;
        private float offMeshTimer;
        private float equipTimer;
        private float moveBackwardsTimer;
        private float getLocationChaseTimer;
        private bool finishedGetLocationChase;

        private Vector3 targetPosOnAttack;
        private Vector3 lastEnemyPos;
        private Vector3 searchPointLocation;
        private Vector3 closestPointToTarget;
        private Vector3 enemyPoint = Vector3.zero;

        private AudioClip lastPlayedChaseAudio;
        ...
        public override void Close()
        {
            ...
            if (blaze.navmeshAgent) blaze.navmeshAgent.avoidancePriority = agentPriority;

            if (blaze.state != BlazeAI.State.attack) blaze.ResetEnemyManager();
        }
        ...
        public override void Main()
        {
            ...
            if (!blaze.navmeshAgent.enabled)
            {
                blaze.PrintWarning(blaze.warnAnomaly,
                    "The Attack State Behaviour is trying to run but the NavMesh Agent component is disabled.");
                return;
            }
#endif
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            if (blaze)
                if (blaze.enemyToAttack)
                {
                    var dir = blaze.enemyColPoint - (transform.position + blaze.vision.visionPosition);
        ...
        public virtual void Engage(Transform target)
        {
            ...
            var distance = blaze.distanceToEnemySqrMag;
            var setDistances = SetDistances();
            var minDistance = setDistances[0];
            var backupDist = setDistances[1];
        ...
        public virtual void IdlePosition()
        {
            ...
            if (turnToTarget)
            {
                var enemyPos = new Vector3(blaze.enemyColPoint.x, transform.position.y, blaze.enemyColPoint.z);
                var toOther = (enemyPos - transform.position).normalized;
                var dotProd = Vector3.Dot(toOther, transform.forward);

                // if AI is currently turning to target
                if (turningToTarget)
                {
                    // TurnTo() turns the AI and returns true when dot == 0.97f and false otherwise
                    if (!blaze.TurnTo(enemyPos, blaze.waypoints.leftTurnAnimAlert, blaze.waypoints.rightTurnAnimAlert,
                            blaze.waypoints.turningAnimT, 7, useTurnAnims)) return;
        ...
        public virtual void Attack()
        {
            ...
            {
                blaze.PrintWarning(blaze.warnAnomaly,
                    "Attack operation skipped because Attack list (property) is empty in Attack State Behaviour. Please add an item to the list. You can even leave the animation empty but atleast set the duration otherwise the duration is unknown and can't attack.");
            ...
            var layers = layersCheckOnAttacking | blaze.vision.hostileAndAlertLayers;
            ...
            if (attacks[index].useAudio)
                if (!blaze.IsAudioScriptableEmpty())
                    blaze.PlayAudio(blaze.audioScriptable.GetAudio(AudioScriptable.AudioType.Attacks,
                        attacks[index].audioIndex));
        ...
        private void AttackTimer()
        {
            ...
            if (startAttackTimer)
            {
                ...
                {
                    if (attackDurationTimer <= 0.1) blaze.RotateTo(targetPosOnAttack, onAttackRotateSpeed);
                }

                // attack timer
                attackDurationTimer += Time.deltaTime;

                if (attackDurationTimer >= attackDuration) StopAttack();
            }
            else
            {
        ...
        private void IntervalAttackTimer()
        {
        ...
        private void AddManagerAndSetInterval(Transform target)
        {
        ...
        private void CheckHeightAndReachable()
        {
            ...
            var enemyColl = GetCollider(blaze.enemyToAttack);
            ...
            if (ranged) enemyPoint = blaze.ValidateYPoint(blaze.enemyColPoint);
        }

        public virtual void ReachableBehaviour(Transform target, float distance, float minDistance, float backupDist)
        {
            if (!CheckMoveBackwardsDone() && !blaze.isAttacking) return;

            if (isReachableCantHit) blaze.StopAttack();
            ...
            if (distance + 0.3f > backupDist * backupDist)
            {
                ...
                if (distance > minDistance * minDistance)
                {
                    ...
                    {
                        var tempDist = distance - minDistance * minDistance;
        ...
        private void TriggerStrafe()
        {
            ...
            if (isStrafeWait || isStrafing) return;
        ...
        public virtual void StrafeMovement(int direction)
        {
            ...
            if (direction == 0)
            {
                ...
                offset = transform.TransformPoint(new Vector3(-(blaze.navmeshAgent.radius + 0.7f), 0f, 0f) +
            ...
            if (strafeCheckPathElapsed >= 5)
            {
                ...
                var goToPoint = blaze.ValidateYPoint(transform.position +
            ...
            // to check from an offset if enemy will not be visible
            if (!blaze.CheckTargetVisibleWithRay(blaze.enemyToAttack.transform, offset, blaze.enemyColPoint - offset,
                    Vector3.Distance(blaze.enemyColPoint, transform.position) + 5, layersToHit))
            {
                StopOrChangeStrafeDirection();
                return;
            }
            ...
            if (_strafingTimer >= _strafeTime) StopStrafe();
        }
        ...
        private void StopOrChangeStrafeDirection()
        {
        ...
        public virtual void MoveBackwards(Vector3 target)
        {
            ...
            var layers = blaze.vision.layersToDetect | agentLayersToCall | strafeLayersToAvoid;
        ...
        private bool CheckMoveBackwardsDone()
        {
            ...
            if (moveBackwards)
                if (moveBackwardsDistance >= blaze.vision.visionDuringAttackState.sightRange)
                {
                    if (blaze.vision.visionDuringAttackState.sightRange - 3 <= 0)
                    {
                        ...
                        Debug.LogWarning(
                            "Move Backwards Distance can't be bigger or equal to Sight Range in Vision During Attack State. Has been reset to 0.");
                        return;
                    }

                    moveBackwardsDistance = blaze.vision.visionDuringAttackState.sightRange - 3;
                    Debug.LogWarning(
                        "Move Backwards Distance can't be bigger or equal to Sight Range in Vision During Attack State.");
                }
        }

        private Collider GetCollider(GameObject go)
        {
        ...
        public virtual void NoTarget()
        {
            ...
            if (!returnedToAlert) blaze.SetState(BlazeAI.State.returningToAlert);
        ...
        private void PlayAttackIdleAudio()
        {
            if (!playAttackIdleAudio || blaze.IsAudioScriptableEmpty()) return;
            ...
            if (attackIdleAudioPlayed) return;
            ...
            if (attackIdleAudTimer < chosenAttackIdleAudTime) return;
        ...
        private void ResetAttackIdleAudio()
        {
        ...
        private void PlayChaseAudio()
        {
            ...
            if (blaze.PlayAudio(lastPlayedChaseAudio)) chaseAudioPlayed = true;
        ...
        private void PlayMoveToAttackAudio()
        {
            if (blaze.IsAudioScriptableEmpty() || !playAudioOnMoveToAttack || moveToAttackAudioPlayed) return;
        ...
        private void ResetMoveToAttackAudio()
        {
