...
namespace BlazeAISpace
{
    ...
    public class AlertTagBehaviour : BlazeBehaviour
    {
        ...
        [Tooltip(
            "Animation name to play the moment the AI sees the alert tag. Will play before moving to location (if checked). If this is empty, it'll be ignored.")]
        ...
        [Tooltip(
            "If check location is true, this animation will play when reaching the location. If check location is false, the animation will play instantly.")]
        ...
        [Min(0)] public float callRange = 10f;
        ...
        [Tooltip(
            "If enabled, the AI will call others not to the exact point but a randomized position within the destination radius. This is to avoid having the AIs all move to the exact same point.")]
        ...
        private bool calledAgents;
        private bool onSightDone;
        private bool isOffMeshBypassed;
        ...
        public override void Main()
        {
            ...
            {
                Debug.Log(
                    "Alert State Behaviour must be added for Alert Tag behaviour to function. Please add the alert state behaviour.");
                return;
            }
            ...
            if (blaze.MoveTo(blaze.sawAlertTagPos, alertStateBehaviour.moveSpeed, alertStateBehaviour.turnSpeed,
                    alertStateBehaviour.moveAnim, alertStateBehaviour.animT))
            {
                blaze.animManager.Play(reachedLocationAnim, animT);
                DurationTimer();
            }
        ...
        {
            if (!showCallRange || !callOtherAgents) return;
        ...
        public virtual void CallAgents()
        {
            ...
            var agentsNum =
                Physics.OverlapSphereNonAlloc(transform.position, callRange, callOthersHitArr, otherAgentsLayers);

            for (var i = 0; i < agentsNum; i++)
            {
                // if this same object then -> skip to next iteration
                if (callOthersHitArr[i].transform.IsChildOf(transform) ||
                    transform.IsChildOf(callOthersHitArr[i].transform)) continue;

                var blazeScript = callOthersHitArr[i].transform.GetComponent<BlazeAI>();
                if (blazeScript == null) continue;

                // set the end point that we'll call the other AIs to
                var point = Vector3.zero;
                var objColl = blaze.sawAlertTagObject.GetComponent<Collider>();
                var range = objColl.bounds.size.x + objColl.bounds.size.z;
                ...
                if (!callPassesColliders)
                {
                    ...
                    var otherAgentCenter = currentColl.ClosestPoint(currentCenter);
                    var dir = otherAgentCenter - currentCenter;

                    var rayDistance = Vector3.Distance(otherAgentCenter, transform.position) + 3;
        ...
        private bool OffMeshToBypass()
        {
            if (!blaze.IfShouldIgnoreOffMesh()) return false;
