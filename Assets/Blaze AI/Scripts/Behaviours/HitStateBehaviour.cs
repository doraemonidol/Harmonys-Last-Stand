...
namespace BlazeAISpace
{
    ...
    public class HitStateBehaviour : BlazeBehaviour
    {
        ...
        [Tooltip("Hit animation names and their durations. One will be chosen at random on every hit.")]
        ...
        [Min(0)]
        [Tooltip(
            "The gap time between replaying the hit animations to avoid having the animation play on every single hit which may look bad on very fast and repitive attacks such as a machine gun.")]
        ...
        [Header("Knock Out Force")]
        [Tooltip(
            "If enabled, on knock out the ragdoll will use the natural velocity of the rigidbody at that moment in time with no additional force.")]
        public bool useNaturalVelocity;

        [Tooltip(
            "If you don't want to use the natural velocity, you can add your own. This can also be changed dynamically through code before calling KnockOut() to add force to the ragdoll depending on the type of hit/weapon.")]
        ...
        [Tooltip(
            "If enabled, a hit audio will always play when hit. If false, there's a 50/50 chance whether an audio will be played or not.")]
        public bool alwaysPlayAudio = true;


        [Header("Call Others")]
        [Min(0)]
        [Tooltip("The radius to call other AIs when hit. You use this by calling blaze.Hit(player, true).")]
        ...
        private Rigidbody hipBoneRb;

        private float elapsedResetBonesTime;
        private float facePos;
        private float standingUpTimer;
        private bool getUpYonBul;
        private string currGetUpAnim;
        private int lastRegisteredKnockOuts;
        ...
        public override void Open()
        {
            ...
            if (blaze.hitProps.callOthers) CallOthers();

            // enable ragdoll if knock out is registered
            if (blaze.hitProps.knockOutRegister != lastRegisteredKnockOuts)
            {
                if (hipBone != null) hipBoneRb = hipBone.GetComponent<Rigidbody>();

                if (bones == null || bones.Length <= 0) PrepareBones();
        ...
        {
            if (cancelAttackOnHit) blaze.StopAttack();
        ...
        public virtual void HitBehaviour()
        {
            ...
            if (_duration >= hitDuration) FinishHitState();
        }
        ...
        private void PlayAudio()
        {
            ...
            if (blaze.PlayAudio(blaze.audioScriptable.GetAudio(AudioScriptable.AudioType.Hit))) playedAudio = true;
        }
        ...
        public virtual void CallOthers()
        {
            ...
            var agentsCollNum = Physics.OverlapSphereNonAlloc(transform.position + blaze.vision.visionPosition,
                callOthersRadius, callOthersHitArr, agentLayersToCall);
            ...
            for (var i = 0; i < agentsCollNum; i++)
            {
                ...
                if (transform.IsChildOf(callOthersHitArr[i].transform)) continue;
                ...
                var randomPoint = script.RandomSpherePoint(transform.position);
        ...
        public virtual void TriggerRagdoll()
        {
            ...
            if (!useNaturalVelocity)
            {
                if (hipBoneRb != null)
                {
                    var dir = transform.TransformDirection(knockOutForce);
                    hipBoneRb.AddForce(dir, ForceMode.Impulse);
                }
                else
                {
                    blaze.PrintWarning(blaze.warnAnomaly,
                        "Hip Bone property in hit state behaviour hasn't been set. No force can be applied to the ragdoll unless you set this.");
                }
            }
        ...
        {
            if (!IsRagdollSleeping()) return;

            _duration += Time.deltaTime;

            if (_duration < knockOutDuration) return;
        ...
        {
            var colls = blaze.GetRagdollColliders();
            var sleepingRB = 0;

            foreach (var c in colls)
                if (Mathf.Abs(c.attachedRigidbody.velocity.x) <= 0.3f &&
                    Mathf.Abs(c.attachedRigidbody.velocity.y) <= 0.3f &&
                    Mathf.Abs(c.attachedRigidbody.velocity.z) <= 0.3f)
                    sleepingRB++;

            if (colls.Count > 1)
            {
                // if half of the rigidbodies are rested then consider ragdoll has finished moving
                if (sleepingRB >= colls.Count / 2) return true;

                return false;
            }

            if (colls.Count == sleepingRB) return true;
        ...
        private void ResettingBonesBehaviour()
        {
            ...
            var elapsedPercentage = elapsedResetBonesTime / ragdollToStandSpeed;
        ...
        private void AlignRotationToHips()
        {
            ...
            var originalHipsPosition = hipBone.position;
            var originalHipsRotation = hipBone.rotation;
        ...
        private void AlignPositionToHips()
        {
            ...
            if (Physics.Raycast(transform.position, Vector3.down, out var hitInfo, blaze.groundLayers))
        ...
        private void PopulateAnimationStartBoneTransforms(string clipName, BoneTransform[] boneTransforms)
        {
            ...
            foreach (var clip in blaze.anim.runtimeAnimatorController.animationClips)
