...
public class BlazeAI : MonoBehaviour
{
    ...
    public virtual void Distract(Vector3 location, bool playAudio = true)
    {
        if (!canDistract || state == State.attack || !enabled || state == State.hit || state == State.death ||
            companionMode) return;


        // get nearest navmesh position
        var pos = GetSamplePosition(ValidateYPoint(location), navmeshAgent.height * 2);
        if (pos == Vector3.zero || pos == endDestination) return;
    ...
    private void CheckDistanceCullingWithState()
    {
    ...
    [Header("General")]
    [Tooltip(
        "Enabling this will make the agent use root motion, this gives more accurate and realistic movement but any move speed property will not be considered as the speed will be that of the animation.")]
    ...
    [Header("Audios")]
    [Tooltip(
        "All audios are added in a scriptable then both Blaze and it's behaviours will read from this scriptable. To create an audio scriptable: Right-click in the Project window > Create > Blaze AI > Audio Scriptable.")]
    public AudioScriptable audioScriptable;

    public AudioSource agentAudio;


    [Header("Waypoints & Turning")] public Waypoints waypoints;


    [Header("Vision & Adding Enemies")] public Vision vision;


    [Header("Check For Enemy Contact")]
    [Tooltip("Check if a hostile got too close and came in contact with the AI. If so, will turn to attack state.")]
    ...
    [Tooltip(
        "Disable this gameobject when it exceeds the distance set in the BlazeAIDistanceCulling component. This will drastically improve performance.")]
    public bool distanceCull;

    [Tooltip(
        "An animation to play when the AI is culled. This will only be considered if BlazeAIDistanceCulling is set to Disable Blaze Only. You can leave this empty if you want.")]
    public string animToPlayOnCull;


    [Header("Unreachable Enemies")]
    [Tooltip(
        "If enabled and an enemy is unreachable or becomes unreachable the AI will ignore it and continue patrolling. If turned off, the AI will not ignore the enemy and turn to attack state and wait for it to be reachable.")]
    public bool ignoreUnreachableEnemy;

    [Tooltip(
        "If an unreachable enemy has been detected, the AI will choose ONE random point from this array to move to in alert state then continue it's patrol. If array length is 0 OR the chosen index == Vector3.zero then the AI will turn to alert state and patrol it's normal waypoints set in the Waypoints class.")]
    ...
    [Header("Off Mesh Links")] public bool useOffMeshLinks;

    public OffMeshLinkJumpMethod jumpMethod = OffMeshLinkJumpMethod.Parabola;

    [Min(0)] public float jumpHeight = 2;

    [Min(0)] public float jumpDuration = 0.8f;

    public bool useMovementSpeedForJump;

    [Min(0)] public float jumpSpeed = 3;

    public string jumpAnim;
    public string fallAnim;

    [Min(0)] public float jumpAnimT = 0.25f;

    public UnityEvent onTeleportStart;
    public UnityEvent onTeleportEnd;

    [Header("Climbing Ladders")] public bool climbLadders;

    public LayerMask ladderLayers;

    [Min(0)] public float climbUpSpeed = 3;

    public string climbUpAnim;

    [Min(0)] public float climbToTopDuration = 1;

    public string climbToTopAnim;

    [Min(0)]
    [Tooltip(
        "The bigger the value, the less head room will be give to initiate the climbing to top animation and vice versa. This value is different for every AI model and you should be trying out different values.")]
    public float climbToTopHeadRoom = 0.4f;

    [Min(0)] public float climbAnimT = 0.25f;


    [Header("Normal State")] public bool useNormalStateOnAwake;

    public MonoBehaviour normalStateBehaviour;


    [Header("Alert State")] public bool useAlertStateOnAwake;
    ...
    [Header("Surprised State")] public bool useSurprisedState;

    public MonoBehaviour surprisedStateBehaviour;


    [Header("Distracted State")] public bool canDistract = true;

    public MonoBehaviour distractedStateBehaviour;

    [Range(0, 100)]
    [Tooltip(
        "If a distraction triggers a group of agents, the highest priority AI only is sent to the distraction point. Here you can set which AI is more prone to check the distraction.")]
    public float priorityLevel = 50;

    [Tooltip(
        "If enabled, the AI will turn to face every distraction it gets. If disabled, the AI turns only once per distracted state duration.")]
    public bool turnOnEveryDistraction = true;

    [Header("Turn To Alert")]
    [Tooltip(
        "If enabled and the AI gets distracted in normal state. It'll play the alert movement animation as well as have the alert vision and when the distracted state is finished, the AI will return to alert state instead of normal. Enabling this option makes the AI act exactly as if it's been distracted during alert state.")]
    public bool turnAlertOnDistract;

    [Header("Audios")]
    [Tooltip("Play audio when distracted. Set the audios in the audio scriptable in the General tab.")]
    public bool playDistractedAudios;


    [Header("Hit State")]
    [Tooltip(
        "If enabled, the AI will react to a max number of hits, after which it'll need to cooldown before reacting to hits again. If you don't want your players to exploit the AI being in a continous hit state this option is great to use. If disabled, the AI will always react to hits.")]
    ...
    [Header("Death")] public string deathAnim;

    [Min(0)] public float deathAnimT = 0.25f;

    [Header("Disable Capsule")] public bool disableCapsuleColliderOnDeath;

    [Header("Audio")] [Tooltip("Set your audios in the audio scriptable in the General Tab in Blaze AI.")]
    public bool playDeathAudio;

    [Header("Call Others")]
    [Min(0)]
    [Tooltip(
        "The radius of calling other AIs on death. The will appear in the scene view as a cyan colored wire sphere.")]
    ...
    [Header("Ragdoll")]
    [Tooltip(
        "If enabled, on death the system will trigger all colliders and disable the animator to have a ragdoll. To have a ragdoll, right click on the AI in the heirarchy > 3D Object > Ragdoll. And follow the wizard.")]
    ...
    [Tooltip(
        "Set manually the force of the rigidbody in ragdoll. You can change this according to the type of death, for example: increase the Y axis when AI is killed by an explosion to have it fly into the air.")]
    public Vector3 deathRagdollForce;

    [Space(15)] [Tooltip("Set an event to trigger on death.")]
    public UnityEvent deathEvent;

    [Header("Destroy")] [Tooltip("Will destroy this gameobject on death.")]
    ...
    [Tooltip(
        "Warns about any anomaly in the AI during gameplay. Like calling an API whos certain conditions are not met.")]
    ...
    private Vector3 lastCalculatedPath;
    private Queue<Vector3> cornersQueue;

    private bool useNormalStateOnAwakeInspectorState;
    private bool useAlertStateOnAwakeInspectorState;
    private bool isturningToCorner;
    private bool isOffMeshJumpFinished = true;
    private bool isChangeStateLooping;

    private BlazeBehaviour lastBlazeBehaviour;

    private int visionCheckElapsed;
    private int closestPointElapsed = 5;
    private int checkSurroundingElapsed;
    private int hitCount;

    private float timeOfLastHit;
    private float offMeshDoneTimePassed = 0.3f;
    private readonly float timeToAllowTurning = 0.3f;

    private string lastAnim;
    private string lastFinishedAnimT;

    private Collider ignoredEnemy;

    private Transform visionT;
    private Transform chosenLadder;

    private readonly List<Collider> ragdollColls = new();
    private Avatar defaultAvatar;

    private DeathDollCallProps deathDollCallProps;
    private LastMoveProps lastMoveProperties;
    private OffMeshDeath offMeshDeathProps;
    ...
    private struct DeathDollCallProps
    {
    ...
    private struct OffMeshDeath
    {
        public bool isCalled;
        public readonly bool callOthers;
        public readonly GameObject enemy;
        public readonly bool comingFromDeathDoll;
    ...
    public struct HitProps
    {
        ...
        public HitProps(bool hitRegister, int knockOutRegister, GameObject hitEnemy, bool callOthers,
            bool hitWhileInCover)
        {
    ...
    private readonly Collider[] visionHitArr = new Collider[20];
    private readonly Collider[] skinHitArr = new Collider[15];
    private readonly RaycastHit[] checkObjVisibleRayHitArr = new RaycastHit[15];
    private List<RaycastHit> orderedRayHits = new();
    private readonly Collider[] searchLadderHitArr = new Collider[7];
    ...
    public virtual void Update()
    {
        ...
        if (state != State.goingToCover) ResetVisionPosition();
    }
    ...
    private void OnValidate()
    {
        ...
        if (!useAlertStateOnAwake && !useNormalStateOnAwake) useNormalStateOnAwake = true;
        ...
        if (waypoints != null) waypoints.WaypointsValidation(transform.position);
        ...
        if (vision != null)
        {
            ...
            if (vision.minSightLevel > vision.visionPosition.y) vision.minSightLevel = -vision.visionPosition.y;
    ...
    private void ComponentsOnAwake()
    {
...
#if UNITY_EDITOR
        if (warnEmptyBehavioursOnStart) CheckEmptyBehaviours();
#endif
        ...
        if (spareState != null) spareState.blaze = this;
    }
    ...
    public virtual void OnDisable()
    {
        ...
        if (animManager != null) animManager.ResetLastState();

        ResetEnemyManager();

        if (audioManager != null) audioManager.RemoveFromManager(this);

        isChangeStateLooping = false;
    }
    ...
    // move to location
    public virtual bool MoveTo(Vector3 location, float moveSpeed, float turnSpeed, string animName = null,
        float animT = 0.25f, string dir = "front", float closestPointDistance = 0)
    {
        ...
        if (dir == "front")
        {
            ...
            if ((!isAttacking || enemyToAttack == null) && lastCalculatedPath == location && cornersQueue.Count == 0)
            {
                ...
                var dist = (new Vector3(pathCorner.x, transform.position.y, pathCorner.z) - transform.position)
                    .sqrMagnitude;
                float minDis = 0;
        ...
        var max = path.corners.Length;
        for (var i = 1; i < max; i++) cornersQueue.Enqueue(path.corners[i]);
    ...
    private bool GoToCorner(string anim, float animT, float moveSpeed, float turnSpeed, string dir)
    {
        var currentDistance = 0f;
        var minDistance = 0f;

        var isLastCorner = false;
        ...
        else
        {
            ...
            currentDistance = (new Vector3(pathCorner.x, transform.position.y, pathCorner.z) - transform.position)
                .sqrMagnitude;
        }
    ...
    public virtual void NonRootMotionMovement(string anim, float animT, float moveSpeed, float turnSpeed, string dir)
    {
        ...
        if (anim != null && anim.Length > 0)
        {
            ...
            if (lastAnim == anim)
            {
                var isDone = CheckAnimTFinished(anim, animT);
                if (!isDone) return;
            }
        }
    ...
    private bool MovementTurning()
    {
        ...
        {
            if (!TurnTo(pathCorner, GetTurnAnim("left"), GetTurnAnim("right"), waypoints.turningAnimT,
                    waypoints.turnSpeed, waypoints.useTurnAnims)) return false;
    ...
    private void MovementRotate(float turnSpeed)
    {
    ...
    private bool IsPathCornerDotProdExtreme(bool onCloseDistanceOnly = false)
    {
        if (onCloseDistanceOnly)
        {
            var distance = CalculateCornersDistanceFrom(transform.position, pathCorner);
            if (distance > Mathf.Clamp(navmeshAgent.radius * 2.2f, 2f, Mathf.Infinity)) return false;
        }

        var dotProd = Vector3.Dot((pathCorner - transform.position).normalized, transform.forward);
        if (dotProd < Mathf.Clamp(waypoints.movementTurningSensitivity, -1, 0.97f)) return true;
    ...
    {
        if (cornersQueue.Count > 0) pathCorner = cornersQueue.Dequeue();
    ...
    {
        var rotationVector =
            (new Vector3(location.x, transform.position.y, location.z) - transform.position).normalized;
        if (rotationVector == Vector3.zero) return;

        var lookRotation = Quaternion.LookRotation(rotationVector);
        ...
        transform.rotation = Quaternion.Slerp(new Quaternion(0f, transform.rotation.y, 0f, transform.rotation.w),
            lookRotation, speed * Time.deltaTime);
    }
    ...
    public bool CheckWayPointRotation()
    {
        ...
        {
            var dotProd = Vector3.Dot(
                (new Vector3(transform.position.x + waypoints.waypointsRotation[waypointIndex].x, transform.position.y,
                    transform.position.z + waypoints.waypointsRotation[waypointIndex].y) - transform.position)
    ...
    public bool WayPointTurning()
    {
        ...
        var leftTurnAnim = GetTurnAnim("left");
        var rightTurnAnim = GetTurnAnim("right");
        var animT = waypoints.turningAnimT;

        if (waypoints.waypointsRotation[waypointIndex].x != 0 || waypoints.waypointsRotation[waypointIndex].y != 0)
        {
            var wayPointDir = new Vector3(transform.position.x + waypoints.waypointsRotation[waypointIndex].x,
                transform.position.y, transform.position.z + waypoints.waypointsRotation[waypointIndex].y);
    ...
    public bool TurnTo(Vector3 location, string leftTurnAnim = null, string rightTurnAnim = null, float animT = 0.25f,
        float turnSpeed = 0, bool playAnims = true)
    {
        location = new Vector3(location.x, transform.position.y, location.z);

        // get dir (left or right)
        var waypointTurnDir = AngleDir
        (transform.forward,
            location - transform.position,
            transform.up);


        var dotProd = Vector3.Dot((location - transform.position).normalized, transform.forward);
        ...
        if (turnSpeed == 0) turnSpeed = waypoints.turnSpeed;
    ...
    private string GetTurnAnim(string dir)
    {
        if (state == State.normal)
        {
            if (dir == "right") return waypoints.rightTurnAnimNormal;
            return waypoints.leftTurnAnimNormal;
        }

        if (dir == "right") return waypoints.rightTurnAnimAlert;
        return waypoints.leftTurnAnimAlert;
    }

    private bool CheckAnimTFinished(string animName, float tTime)
    {
        if (animName == lastFinishedAnimT) return true;
    ...
    public virtual void VisionCheck()
    {
        ...
        {
            if (Array.IndexOf(vision.hostileTags, enemyToAttack.tag) < 0 || !enemyToAttack.activeSelf)
            {
        ...
        var npcDir = transform.position + vision.visionPosition;
        ...
        Array.Clear(visionHitArr, 0, 20);
        var visionHitArrNum =
            Physics.OverlapSphereNonAlloc(transform.position, range, visionHitArr, vision.hostileAndAlertLayers);
        var smallestDist = Mathf.Infinity;
        float testDist = 0;
        var bestIndex = -1;

        for (var i = 0; i < visionHitArrNum; i++)
        {
            if (transform.IsChildOf(visionHitArr[i].transform) ||
                visionHitArr[i].transform.IsChildOf(transform)) continue;

            // if companion mode is on -> stop AI from targeting companion 
            if (companionMode && companionTo != null && companionTo.IsChildOf(visionHitArr[i].transform)) continue;

            // check angle and heights
            var hostile = visionHitArr[i];
            var targetCaughtPoint = hostile.ClosestPoint(transform.position + vision.visionPosition);

            if (Vector3.Angle(visionT.forward, targetCaughtPoint - npcDir) > angle * 0.5f) continue;

            // check if height too high
            if (enemyToAttack == null || (vision.checkTargetHeight && enemyToAttack != null))
            {
                var suspectHeight = targetCaughtPoint.y + vision.visionPosition.y -
                                    (transform.position.y + vision.visionPosition.y + vision.maxSightLevel);
                if (suspectHeight > 0) continue;
            }
            ...
            {
                var alertTagIndex = vision.GetAlertTagIndex(visionHitArr[i].tag);
                if (alertTagIndex >= 0)
                {
                    var alertObj = visionHitArr[i].transform.gameObject;
            ...
            if (Array.IndexOf(vision.hostileTags, visionHitArr[i].tag) < 0) continue;
            ...
            var enemyToAttackColliders = hostile.transform.GetComponentsInChildren<Collider>();
            var colSize = enemyToAttackColliders.Length;
        ...
        if (ignoreUnreachableEnemy)
            if (!IsPathReachable(visionHitArr[bestIndex].transform.position))
            {
                ...
                if (ignoredEnemy != null)
                {
                    ...
                    if (Array.IndexOf(visionHitArr, ignoredEnemy) >= 0) return;
        ...
        if (previousEnemy != null && previousEnemy != visionHitArr[bestIndex].transform) ResetEnemyManager();

        if (vision.useVisionMeter && enemyToAttack == null)
            if (!CheckVisionMeter(true, visionHitArr[bestIndex].transform.gameObject, range))
                return;
    ...
    public virtual void SawAlertTag(GameObject alertObj, int index)
    {
        ...
        var objectColl = sawAlertTagObject.GetComponent<Collider>();
        sawAlertTagPos = GetSamplePosition(ValidateYPoint(alertObj.transform.position),
            objectColl.bounds.size.x + objectColl.bounds.size.z);


        var layers = vision.layersToDetect | vision.hostileAndAlertLayers;
    ...
    private bool RayCastObjectColliders(GameObject go, int layersToHit, int minDetectionScore)
    {
        var npcDir = transform.position + vision.visionPosition;
        Vector3 colDir;
        ...
        {
            var item = go.GetComponent<Collider>();
            colDir = item.ClosestPoint(transform.position + vision.visionPosition) - npcDir;
            var rayDistance = Vector3.Distance(go.transform.position, transform.position + vision.visionPosition) + 5;

            if (CheckTargetVisibleWithRay(go.transform, npcDir, colDir, rayDistance, layersToHit)) return true;

            return false;
        }


        var objColls = go.transform.GetComponentsInChildren<Collider>();
        var colSize = objColls.Length;
        ...
        for (var i = 0; i < colSize; i++)
        {
            ...
            colDir = item.ClosestPoint(transform.position + vision.visionPosition) - npcDir;
            var rayDistance = Vector3.Distance(item.transform.position, transform.position) + 5;

            // check center
            if (CheckTargetVisibleWithRay(go.transform, npcDir, colDir, rayDistance, layersToHit)) detectionScore++;
            ...
            // checking top right
            colDir = item.ClosestPoint(new Vector3(item.bounds.center.x - item.bounds.extents.x,
                item.bounds.center.y + item.bounds.extents.y, item.bounds.center.z + item.bounds.extents.z)) - npcDir;
            if (CheckTargetVisibleWithRay(go.transform, npcDir, colDir, rayDistance, layersToHit)) detectionScore++;
        }
    ...
    public bool CheckTargetVisibleWithRay(Transform target, Vector3 startDir, Vector3 targetDir, float rayDistance,
        int layers)
    {
        var hits = Physics.RaycastAll(startDir, targetDir, rayDistance, layers);
        var orderedRayHits = new List<RaycastHit>(hits);

        orderedRayHits.Sort((p1, p2) => p1.distance.CompareTo(p2.distance));
        var max = orderedRayHits.Count;

        for (var i = 0; i < max; i++)
        {
            if (orderedRayHits[i].transform.IsChildOf(transform) ||
                transform.IsChildOf(orderedRayHits[i].transform)) continue;

            if (orderedRayHits[i].transform.IsChildOf(target) ||
                target.IsChildOf(orderedRayHits[i].transform)) return true;
    ...
    private float PreviousStateVAngle()
    {
        if (previousState == State.normal) return vision.visionDuringNormalState.coneAngle;

        if (previousState == State.alert) return vision.visionDuringAlertState.coneAngle;

        if (previousState == State.attack) return vision.visionDuringAttackState.coneAngle;

        if (previousState == State.hit)
        {
            if (enemyToAttack) return vision.visionDuringAttackState.coneAngle;
    ...
    private float PreviousStateVRange()
    {
        if (previousState == State.normal) return vision.visionDuringNormalState.sightRange;

        if (previousState == State.alert) return vision.visionDuringAlertState.sightRange;

        if (previousState == State.attack) return vision.visionDuringAttackState.sightRange;

        if (previousState == State.hit)
        {
            if (enemyToAttack) return vision.visionDuringAttackState.sightRange;
    ...
    private void SurroundingsCheck()
    {
        // return if any of these conditions are true
        if (!checkEnemyContact || friendly || enemyToAttack || state == State.hit || state == State.death) return;

        var closeTarget = CheckSurroundingForTarget();
        ...
        if (companionMode && companionTo != null && companionTo.IsChildOf(closeTarget.transform)) return;


        // if caught collider is a child of the same AI -> skip
        if (transform.IsChildOf(closeTarget.transform) || closeTarget.transform.IsChildOf(transform)) return;
            ...
            if (!IsPathReachable(closeTarget.transform.position))
            {
                ...
                if (ignoredEnemy != null)
                {
                    ...
                    if (ignoredEnemy.transform.IsChildOf(closeTarget.transform) ||
                        closeTarget.transform.IsChildOf(ignoredEnemy.transform)) return;
    ...
    private GameObject CheckSurroundingForTarget()
    {
        if (state == State.attack || state == State.goingToCover || state == State.surprised) return null;

        checkSurroundingElapsed++;
        if (checkSurroundingElapsed < 5) return null;
        checkSurroundingElapsed = 0;

        Array.Clear(skinHitArr, 0, 15);
        var numColliders = Physics.OverlapSphereNonAlloc(transform.position + vision.visionPosition, enemyContactRadius,
            skinHitArr, vision.hostileAndAlertLayers);

        for (var i = 0; i < numColliders; i++)
        {
            if (skinHitArr[i].transform.IsChildOf(transform) || transform.IsChildOf(skinHitArr[i].transform)) continue;

            if (Array.IndexOf(vision.hostileTags, skinHitArr[i].transform.tag) >= 0)
            {
                enemyPosOnSurprised = skinHitArr[i].transform.position;

                if (RayCastObjectColliders(skinHitArr[i].transform.gameObject,
                        vision.layersToDetect | vision.hostileAndAlertLayers, 1))
    ...
    // ignore passed enemy
    private void IgnoreEnemy(Collider enemyColl)
    {
        enemyToAttack = null;
        isAttacking = false;
        ...
        else
        {
            ...
            if (state == State.hit || state == State.surprised || state == State.death) return;

            if (state != State.alert) SetState(State.alert);
        }
        ...
        var chosenPoint = fallBackPoints[Random.Range(0, fallBackPoints.Length)];
    ...
    public bool CheckObjectCompletelyVisible(GameObject go, int layersToHit, int minDetectionScore)
    {
        ...
        var objColl = go.transform.GetComponentInChildren<Collider>();
        var objColPoint = objColl.ClosestPoint(transform.position + vision.visionPosition);

        Vector3 npcDir;
        Vector3 colDir;

        Array.Clear(checkObjVisibleRayHitArr, 0, 15);

        var detectionScore = 0;
        var xSide = Mathf.Clamp((objColPoint - transform.position).normalized.x, 0.1f, navmeshAgent.radius / 2);


        // fired from AIs right side
        npcDir = transform.TransformPoint(new Vector3(xSide, 0f, 0f) + vision.visionPosition);
        colDir = objColPoint - npcDir;

        var hits = Physics.RaycastNonAlloc(npcDir, colDir, checkObjVisibleRayHitArr, distanceToEnemy + 5, layersToHit);
        var smallestDist = Mathf.Infinity;
        var bestIndex = -1;

        for (var i = 0; i < hits; i++)
        {
            if (transform.IsChildOf(checkObjVisibleRayHitArr[i].transform) ||
                checkObjVisibleRayHitArr[i].transform.IsChildOf(transform)) continue;

            if (checkObjVisibleRayHitArr[i].distance == 0 ||
                checkObjVisibleRayHitArr[i].point == Vector3.zero) continue;
        ...
        if (bestIndex > -1)
            if (go.transform.IsChildOf(checkObjVisibleRayHitArr[bestIndex].transform) ||
                checkObjVisibleRayHitArr[bestIndex].transform.IsChildOf(go.transform))
                detectionScore++;
        ...
        for (var i = 0; i < hits; i++)
        {
            ...
            if (checkObjVisibleRayHitArr[i].distance == 0) continue;
        ...
        hits = Physics.SphereCastNonAlloc(npcDir, 0.1f, colDir.normalized, checkObjVisibleRayHitArr,
            distanceToEnemy + 5, layersToHit);
    ...
    private void RunEnemyOnEnterEvent()
    {
        if (enemyToAttack != null) return;
        vision.enemyEnterEvent.Invoke();
    }

    private void RunEnemyOnExitEvent()
    {
        if (enemyToAttack == null) return;
        vision.enemyLeaveEvent.Invoke();
    }

    private bool CheckVisionMeter(bool isDetected, GameObject potentialEnemy = null, float currentVisionRange = 0)
    {
        float speed;

        if (isDetected)
        {
            var halfVisionRange = currentVisionRange / 2;
    ...
    public void ResetEnemyManager(bool shouldRemoveEnemy = true)
    {
        ...
        if (shouldRemoveEnemy) enemyManager.RemoveEnemy(this);

        enemyManager = null;
    }

    private void MovePotentialToTargetEnemyManager()
    {
        enemyManager.AddTarget(this);
    ...
    {
        if (!Enum.IsDefined(typeof(State), stateToTurnTo))
        {
    ...
    private void CheckState(bool forceChange = false)
    {
        if ((navmeshAgent.isOnOffMeshLink && !forceChange) || isChangeStateLooping) return;

        isChangeStateLooping = true;
        ...
        switch (state)
        {
            case State.normal:
                if (companionMode)
                {
                    RunBehaviour(companionBehaviour);
                    break;
                }

                RunBehaviour(normalStateBehaviour);
                break;
            case State.alert:
                if (companionMode)
                {
                ...
                RunBehaviour(alertStateBehaviour);
                break;
            case State.attack:
                if (coverShooterMode)
                {
                    RunBehaviour(coverShooterBehaviour);
                    break;
                }

                RunBehaviour(attackStateBehaviour);
                break;
            case State.sawAlertTag:
                RunBehaviour(vision.alertTags[vision.GetAlertTagIndex(sawAlertTagName)].behaviourScript);
                break;
            case State.distracted:
                RunBehaviour(distractedStateBehaviour);
                break;
            case State.surprised:
                RunBehaviour(surprisedStateBehaviour);
                break;
            case State.goingToCover:
                RunBehaviour(goingToCoverBehaviour);
                break;
            case State.hit:
                RunBehaviour(hitStateBehaviour);
                break;
            case State.returningToAlert:
                if (coverShooterMode)
                {
    ...
    // run behaviour current state
    private void RunBehaviour(MonoBehaviour passedBehaviour)
    {
        if (passedBehaviour == null) return;

        if (passedBehaviour == lastBlazeBehaviour)
        {
            lastBlazeBehaviour.Main();
            return;
        }

        // useful if behaviour script changed programmatically then disable that previous one
        if (lastBlazeBehaviour != null) lastBlazeBehaviour.Close();

        lastBlazeBehaviour = null;
        var blazeBehaviours = passedBehaviour.GetComponents<BlazeBehaviour>();

        foreach (var item in blazeBehaviours)
        {
            if (passedBehaviour != item) continue;

            lastBlazeBehaviour = item;
            break;
        }

        if (lastBlazeBehaviour != null) lastBlazeBehaviour.Open();
    }

    // close last behaviour
    public void CloseLastBehaviour()
    {
        if (lastBlazeBehaviour == null) return;
        lastBlazeBehaviour.Close();
        lastBlazeBehaviour = null;
    }

    private void RemoveMoveToLocation()
    {
        if (state != State.alert && state != State.normal) IgnoreMoveToLocation();
    }
    ...
    private void ShowEnemyContactRadius()
    {
        if (!showEnemyContactRadius || !checkEnemyContact) return;

        Gizmos.color = Color.grey;
        Gizmos.DrawWireSphere(transform.position + vision.visionPosition, enemyContactRadius);
    }

    private void ValidateFallBackPoints()
    {
        if (fallBackPoints == null) return;

        for (var i = 0; i < fallBackPoints.Length; i++)
        {
            if (fallBackPoints[i] != Vector3.zero) return;
...
#if UNITY_EDITOR
    private void DrawFallBackPoints()
    {
        ...
        if (groundLayers.value == 0)
            Debug.LogWarning(
                "Ground layers property not set. Make sure to set the ground layers in the main Blaze inspector (general tab) in order to see the fallback points visually.");

        for (var i = 0; i < fallBackPoints.Length; i++)
        {
            RaycastHit hit;
            var point = fallBackPoints[i];
            ...
            if (Physics.Raycast(point, -Vector3.up, out hit, Mathf.Infinity, groundLayers))
            {
                ...
                Handles.DrawWireDisc(hit.point, transform.up, 0.5f);
                Handles.Label(hit.point + new Vector3(0, 1, 0), "Fallback Point");
            }
        }
    }
#endif
    ...
    public virtual void Surprised()
    {
        ...
        if (enemyToAttack) enemyPosOnSurprised = enemyToAttack.transform.position;
    ...
    {
        if (state == State.death || !enabled) return;
        ...
        if (turnStateToAttack) SetState(State.attack);
    }

    // returns whether the AI is a companion to the passed gameobject
    private bool IsCompanion(GameObject enemy)
    {
        if (enemy == null) return false;

        // if companion mode is on -> eliminate the companion from targeting 
        if (companionMode && companionTo != null && enemy.transform.IsChildOf(companionTo)) return true;
    ...
    {
        var result = targetLocation + transform.forward;
        var chosenRange = Random.Range(0.1f, range);
    ...
    private void TurnToHitState(string typeOfHit = "hit", GameObject enemy = null, bool callOthers = false)
    {
        ...
        if (typeOfHit == "hit") hitProps.hitRegister = true;
        ...
        if (state == State.goingToCover && tookCover) hitProps.hitWhileInCover = true;
    ...
    private void CleanKnockOutRegister()
    {
        if (state != State.hit) hitProps.knockOutRegister = 0;
    }

    private void ManageHitsCooldown()
    {
    ...
    private void ShowDeathCallRadius()
    {
        if (!showDeathCallRadius) return;
    ...
    private void CallOthersOnDeath(bool callOthers, GameObject enemy)
    {
        // only run if set to do so
        if (callOthers)
        {
            var agentsColl = new Collider[5];
            var agentsCollNum = Physics.OverlapSphereNonAlloc(transform.position, deathCallRadius, agentsColl,
                agentLayersToDeathCall);

            for (var i = 0; i < agentsCollNum; i++)
            {
                var script = agentsColl[i].GetComponent<BlazeAI>();

                // if caught collider is that of the same AI -> skip
                if (agentsColl[i].transform.IsChildOf(transform)) continue;
                ...
                if (script.state == State.attack || script.state == State.goingToCover ||
                    script.enemyToAttack != null) continue;
    ...
    private void RagdollOrDeathAnim()
    {
        // use death animation
        if (!useRagdoll)
        {
        ...
        if (useRagdoll && ragdollColls.Count <= 0) CollectRagdollColliders();
        ...
        if (hipBone == null)
        {
            PrintWarning(warnAnomaly,
                "Hip Bone property in death tab hasn't been set. No force can be applied to the ragdoll unless you set this.");
            return;
        }

        var rb = hipBone.GetComponent<Rigidbody>();
        if (rb == null)
        {
        ...
        var dir = transform.TransformDirection(deathRagdollForce);
        rb.AddForce(dir, ForceMode.Impulse);
    }

    private void DeathDollCall()
    {
        Death(deathDollCallProps.callOthers, deathDollCallProps.enemy, true);
    }

    private void CancelDeathDoll()
    {
        CancelInvoke("DeathDollCall");
    }

    private bool CheckDeathOnOffMesh()
    {
    ...
    public virtual void CollectRagdollColliders()
    {
        ...
        foreach (var c in coll)
        {
            ...
            if (ragdollColls.Contains(c)) continue;

            ragdollColls.Add(c);

            var script = c.gameObject.AddComponent(typeof(BlazeAIRagdollData)) as BlazeAIRagdollData;
    ...
    public virtual void DisableRagdoll(bool reset = false)
    {
        foreach (var c in ragdollColls)
        {
            ...
            if (reset)
            {
                var script = c.transform.GetComponent<BlazeAIRagdollData>();
    ...
    public Transform[] GetRagdollTransforms()
    {
        ...
        foreach (var c in ragdollColls) list.Add(c.transform);
    ...
    private void CheckEmptyBehaviours()
    {
        if (useNormalStateOnAwake)
            if (normalStateBehaviour == null)
                PrintWarning(warnEmptyBehavioursOnStart,
                    $"Normal State Behaviour is empty in game object: {gameObject.name}.");


        if (useAlertStateOnAwake)
            if (alertStateBehaviour == null)
                PrintWarning(warnEmptyBehavioursOnStart,
                    $"Alert State Behaviour is empty in game object: {gameObject.name}.");


        if (canDistract)
            if (distractedStateBehaviour == null)
                PrintWarning(warnEmptyBehavioursOnStart,
                    $"Distracted State Behaviour is empty in game object: {gameObject.name}.");


        if (useSurprisedState)
            if (surprisedStateBehaviour == null)
                PrintWarning(warnEmptyBehavioursOnStart,
                    $"Surprised State Behaviour is empty in game object: {gameObject.name}.");
        ...
        if (!coverShooterMode)
        {
            if (attackStateBehaviour == null)
                PrintWarning(warnEmptyBehavioursOnStart,
                    $"Attack State Behaviour is empty in game object: {gameObject.name}.");
        }
        else
        {
            if (coverShooterBehaviour == null)
                PrintWarning(warnEmptyBehavioursOnStart,
                    $"Cover Shooter Behaviour is empty in game object: {gameObject.name}.");

            if (goingToCoverBehaviour == null)
                PrintWarning(warnEmptyBehavioursOnStart,
                    $"Going To Cover Behaviour is empty in game object: {gameObject.name}.");
        }


        if (hitStateBehaviour == null)
            PrintWarning(warnEmptyBehavioursOnStart,
                $"Hit State Behaviour is empty in game object: {gameObject.name}.");
    }
    ...
    public void SetPrimeBehaviours()
    {
        ...
        var normalBehaviour = GetComponent<NormalStateBehaviour>();
        ...
        var alertBehaviour = GetComponent<AlertStateBehaviour>();
        ...
        var attackBehaviour = GetComponent<AttackStateBehaviour>();
        ...
        var shooterBehaviour = GetComponent<CoverShooterBehaviour>();
        ...
        var goingBehaviour = GetComponent<GoingToCoverBehaviour>();
    ...
    private void SetSurprisedBehaviour()
    {
        var surprisedBehaviour = GetComponent<SurprisedStateBehaviour>();
    ...
    {
        var distractedBehaviour = GetComponent<DistractedStateBehaviour>();
    ...
    {
        var hitBehaviour = GetComponent<HitStateBehaviour>();
    ...
    {
        var cb = GetComponent<CompanionBehaviour>();

        if (cb == null) cb = gameObject.AddComponent(typeof(CompanionBehaviour)) as CompanionBehaviour;
    ...
    public void PlayPatrolAudio()
    {
        ...
        if (state == State.normal)
        {
            var stateBehaviour = (NormalStateBehaviour)normalStateBehaviour;
        ...
        {
            var stateBehaviour = (AlertStateBehaviour)alertStateBehaviour;
            if (stateBehaviour.playPatrolAudio)
                PlayAudio(audioScriptable.GetAudio(AudioScriptable.AudioType.AlertState));
        }
    }

    private void SetAgentAudio()
    {
        SetAudioManager();
        ...
        if (agentAudio == null) agentAudio = gameObject.AddComponent(typeof(AudioSource)) as AudioSource;
    ...
    public bool IsAudioScriptableEmpty()
    {
        if (audioScriptable == null)
        {
            if (warnEmptyAudio)
                Debug.LogWarning(
                    "A behaviour checked for an audio scriptable to play an audio but the property was empty.");
            return true;
        }
    ...
    public virtual Vector3 RandomNavMeshLocation(float minLevel, float maxLevel)
    {
        if (navmeshAgent == null) return transform.position;

        var randomDirection = Random.insideUnitSphere * waypoints.randomizeRadius;
        randomDirection += StartPosition;

        NavMeshHit hit;
        var point = Vector3.zero;

        var radius = navmeshAgent.radius * 2;
        float distance;
        var isChosen = false;
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            distance = (new Vector3(point.x, transform.position.y, point.y) - transform.position).sqrMagnitude;
            if (distance < radius * radius) continue;

            if (!IsPathReachable(point)) continue;

            var heightProjection = Mathf.Abs(point.y) - Mathf.Abs(ValidateYPoint(StartPosition).y);
            if (heightProjection != 0 && (heightProjection < minLevel || heightProjection > maxLevel)) continue;
    ...
    public virtual Vector3 ValidateYPoint(Vector3 pos)
    {
        if (!IsPointOnNavMesh(pos, 0.3f))
        {
            ...
            if (Physics.Raycast(pos, -Vector3.up, out downHit, Mathf.Infinity, groundLayers)) return downHit.point;
        }
    ...
    public virtual bool IsPathReachable(Vector3 position, bool addAsLastCalcPath = false)
    {
        ...
        if (position.x == Mathf.Infinity || position.z == Mathf.Infinity || position.y == Mathf.Infinity) return false;

        // set layer of path calculation
        var pathBit = NavMesh.AllAreas;
        if (!useOffMeshLinks) pathBit = 1;

        NavMesh.CalculatePath(ValidateYPoint(transform.position), ValidateYPoint(position), pathBit, path);

        // used in movement
        if (addAsLastCalcPath) lastCalculatedPath = position;
    ...
    public virtual Vector3 RandomSpherePoint(Vector3 point, float range = -1, bool divideRange = true)
    {
        ...
        var random = point + Random.insideUnitSphere.normalized * range;

        if (divideRange) return GetSamplePosition(random, range / 2);
    ...
    public virtual float CalculateCornersDistanceFrom(Vector3 beginLocation, Vector3 destination)
    {
        ...
        NavMesh.CalculatePath(ValidateYPoint(beginLocation), ValidateYPoint(destination), NavMesh.AllAreas,
            calculatedPath);

        var distance = 0f;
        var max = calculatedPath.corners.Length;
    ...
    public virtual void OffMeshLinkJump(float speed)
    {
        ...
        {
            if (isOffMeshJumpFinished) StartCoroutine(Parabola(navmeshAgent, jumpHeight, jumpDuration));
        }
        else
        {
    ...
    public bool IfShouldIgnoreOffMesh()
    {
        if (useOffMeshLinks) return false;
        if (!IsOnOffMeshLink()) return false;
        return true;
    }

    private IEnumerator NormalSpeed(NavMeshAgent agent, float speed, bool climbing = false)
    {
        isOffMeshJumpFinished = false;

        var data = agent.currentOffMeshLinkData;
        var endPos = data.endPos + Vector3.up * agent.baseOffset;
        var headRoom = agent.height / 2 / 2 - climbToTopHeadRoom;

        if (climbing) endPos = new Vector3(transform.position.x, endPos.y + headRoom, transform.position.z);

        while (agent.transform.position != endPos)
        {
            if (ConditionsToBreakOffMeshLink()) break;
    ...
    private void JumpEnd(Vector3 endPos, NavMeshAgent agent)
    {
    ...
    private void TurningTimerAfterOffMesh()
    {
        if (navmeshAgent.isOnOffMeshLink) return;
        if (!isOffMeshJumpFinished) return;

        if (offMeshDoneTimePassed < timeToAllowTurning) offMeshDoneTimePassed += Time.deltaTime;
    }

    private bool ConditionsToBreakOffMeshLink()
    {
        if (state == State.death && !deathDollCallProps.isCalled && useRagdoll) return true;

        if (state == State.hit && !hitProps.hitRegister) return true;

        return false;
    }

    private void PlayOffMeshAnim()
    {
        var data = navmeshAgent.currentOffMeshLinkData;
        var endPos = data.endPos + Vector3.up * navmeshAgent.baseOffset;
    ...
    private bool SearchForLadder()
    {
        if (!climbLadders) return false;
        if (navmeshAgent.currentOffMeshLinkData.endPos.y <= transform.position.y) return false;

        Array.Clear(searchLadderHitArr, 0, 7);
        var numColliders = Physics.OverlapSphereNonAlloc(transform.position + vision.visionPosition,
            navmeshAgent.radius + 0.3f, searchLadderHitArr, ladderLayers);

        for (var i = 0; i < numColliders; i++)
        {
            if (searchLadderHitArr[i].transform.IsChildOf(transform) ||
                transform.IsChildOf(searchLadderHitArr[i].transform)) continue;
    ...
    private void ClimbLadderBehaviour()
    {
        if (!isOffMeshJumpFinished) return;
    ...
    public virtual void MoveToLocation(Vector3 location, bool randomize = false)
    {
        ...
        else
        {
            ...
            if (waypointIndex >= 0 && !movedToLocation) waypointIndex--;
        }
    ...
    public virtual bool IsIdle()
    {
        ...
        {
            PrintWarning(warnAnomaly,
                "IsIdle() only works when the AI is in normal, alert and distracted states. Will return false.");
            return false;
        }
    ...
    public virtual void Attack()
    {
        ...
        if (state != State.hit) SetState(State.attack);
    }
    ...
    public virtual void ChangeState(string stateStr)
    {
        ...
        if (state == State.death && hipBone != null && useRagdoll) navmeshAgent.Warp(hipBone.position);
        ...
        if (state == State.death)
        {
            if (hipBone != null) hipBone.position = transform.position;
        ...
        if (stateStr == "normal") SetState(State.normal);

        if (stateStr == "alert") SetState(State.alert);
    }
    ...
    public virtual void SetTarget(GameObject enemy, bool randomizePoint = false, bool applyAttackVisionForFrame = false)
    {
        ...
        {
            PrintWarning(warnAnomaly,
                "Can't call SetTarget() when AI is in death state or Blaze AI is disabled. You have to call ChangeState(string state) first to revive the AI.");
            return;
        }
    ...
    public virtual void KnockOut(GameObject enemy = null, bool callOthers = false)
    {
        ...
        {
            PrintWarning(warnAnomaly,
                "KnockOut() can't be called when the AI is in death state or Blaze AI is disabled.");
            return;
        }

        // collect colliders if list is empty
        if (ragdollColls.Count <= 0) CollectRagdollColliders();
    ...
    public virtual void Death(bool callOthers = false, GameObject enemy = null, bool comingFromDeathDoll = false)
    {
        ...
        if (!comingFromDeathDoll)
            ...
            {
                PrintWarning(warnAnomaly,
                    "Death() can't be called when the AI is in death state or Blaze AI is disabled.");
                return;
            }
        ...
        if (capsuleCollider != null && disableCapsuleColliderOnDeath) capsuleCollider.enabled = false;
    ...
    public virtual void DeathDoll(float timeToRagdoll, bool callOthers = false, GameObject enemy = null)
    {
        ...
        {
            PrintWarning(warnAnomaly,
                "DeathDoll() can't be called when the AI is in death state or Blaze AI is disabled.");
            return;
        }
    ...
    public virtual void AddDistanceCulling()
    {
        ...
        PrintWarning(warnAnomaly,
            "Can't add agent to distance culling list since no distance culling instance can be found.");
    }
    ...
    public virtual void RemoveDistanceCulling(bool enableObject = false)
    {
        if (BlazeAIDistanceCulling.instance)
        {
            ...
            if (enableObject) gameObject.SetActive(true);
        }
    }
    ...
    {
        if (BlazeAIDistanceCulling.instance) return BlazeAIDistanceCulling.instance.CheckAgent(this);
    ...
    public virtual void SetSpareState(string stateName, int animIndex = -1, int audioIndex = -1)
    {
        ...
        {
            PrintWarning(warnAnomaly,
                "Can't set spare state as the Blaze AI Spare State component hasn't been added to the AI. Please add the component first.");
            return;
        }
        ...
        {
            PrintWarning(warnAnomaly,
                "SetSpareState() can't be called when the AI is in death state or Blaze AI is disabled.");
            return;
        }
        ...
        {
            PrintWarning(warnAnomaly,
                "Blaze is already in spare state. Current spare state must finish first before calling another one.");
            return;
        }
