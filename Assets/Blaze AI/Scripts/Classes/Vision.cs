...
namespace BlazeAISpace
{
    ...
    public class Vision
    {
        ...
        [Tooltip(
            "This is where the vision ray will start from. Place on the eyes of the AI. Enable [Show Vision] below to draw the vision cone in the scene view for easy placement. If [Max Sight Level] is set to 0. This Y point will be the maximum Y position the AI can detect and anything bigger will not be detected or seen.")]
        public Vector3 visionPosition = new(0, 1, 0);

        [Min(0f)]
        [Tooltip(
            "The maximum Y position the AI can detect. If any target's Y position is bigger than this, it won't be detected or seen. Enable [Show Max Sight Level] below to show as a green rectangle in the scene view.")]
        ...
        [Tooltip(
            "The minimum Y position the AI can detect. If any target's Y position is lower than this, it won't be detected or seen. Enable [Show Min Sight Level] below to show as a red rectangle in the scene view.")]
        public float minSightLevel = -1;


        [Header("Vision Range And Angle For Each State")]
        public normalVision visionDuringNormalState = new(90f, 10f);

        public alertVision visionDuringAlertState = new(100f, 15f);
        public attackVision visionDuringAttackState = new(360f, 20f);


        [Header("Sync Vision-Head Rotation (optional)")]
        [Tooltip(
            "OPTIONAL: add the head object, this will be used for updating both the rotation of the vision according to the head and the sight level automatically. If empty, the rotation will be according to the body, projecting forwards.")]
        ...
        [Tooltip(
            "Add all the layers you want to detect around the world. Any layer not added will be seen through. Recommended not to add other Blaze agents layers in order for them not to block the view from your target. Also no need to set the enemy layers here.")]
        public LayerMask layersToDetect = Physics.AllLayers;

        [Tooltip(
            "Set the layers of the hostiles and alerts. Hostiles are the enemies you want to attack. Alerts are objects when seen will turn the AI to alert state.")]
        public LayerMask hostileAndAlertLayers;

        [Tooltip(
            "The tag names of hostile gameobjects (player) that this agent should attack. This needs to be set in order to identify enemies.")]
        public string[] hostileTags;

        [Tooltip(
            "Optional: Tags that will make the agent become in alert state such as tags of dead bodies or an open door.")]
        ...
        [Header("Vision Meter")]
        [Tooltip(
            "Instead of detecting the enemy immediately, the vision meter will increment until it's complete then the enemy gets detected.")]
        public bool useVisionMeter;

        public VisionMeterSpeeds visionMeterSpeeds = new(5, 10, 3);


        #region STRUCTS

        [Serializable]
        public struct normalVision
        {
            [Range(0f, 360f)] public float coneAngle;
            [Min(0f)] public float sightRange;
        ...
        public struct AlertTags
        {
            ...
            [Tooltip(
                "When the AI sees an object with an alert tag it'll immediately change it to this value. In order not to get alerted by it again. If this value is empty it'll fall back to 'Untagged'.")]
            public string fallBackTag;
        }

        [Serializable]
        public struct VisionMeterSpeeds
        {
            [Min(0)]
            [Tooltip("Set the speed of detection when the target's distance is > than half the vision radius.")]
        ...
        {
            if (showNormalVision)
                DrawVisionCone(visionTransform, charTransform, visionDuringNormalState.coneAngle,
                    visionDuringNormalState.sightRange, Color.white);

            if (showAlertVision)
                DrawVisionCone(visionTransform, charTransform, visionDuringAlertState.coneAngle,
                    visionDuringAlertState.sightRange, Color.white);

            if (showAttackVision)
                DrawVisionCone(visionTransform, charTransform, visionDuringAttackState.coneAngle,
                    visionDuringAttackState.sightRange, Color.red);

            if (showMaxSightLevel)
                // all the passed arguments are being ignored
                DrawVisionCone(visionTransform, charTransform, visionDuringAttackState.coneAngle,
                    visionDuringAttackState.sightRange, Color.red, true);
        ...
        // draw vision cone
        private void DrawVisionCone(Transform visionTransform, Transform charTransform, float angle, float rayRange,
            Color color, bool ignore = false)
        {
            ...
            if (ignore)
            {
                if (showMaxSightLevel && maxSightLevel > 0f)
                {
                    ...
                    Gizmos.DrawCube(
                        charTransform.forward + charTransform.position +
                        new Vector3(visionPosition.x, maxSightLevel, visionPosition.z), new Vector3(0.5f, 0.05f, 0.5f));
                }
                ...
                if (showMinSightLevel)
                {
                    ...
                    Gizmos.DrawCube(
                        charTransform.forward + charTransform.position +
                        new Vector3(visionPosition.x, minSightLevel, visionPosition.z), new Vector3(0.5f, 0.05f, 0.5f));
                }
            ...
            if (angle >= 360f)
            {
                ...
                Gizmos.DrawWireSphere(
                    visionTransform.position + new Vector3(visionPosition.x, visionPosition.y + maxSightLevel,
                        visionPosition.z), rayRange);
                return;
            }
            ...
            var leftRayRotation1 = Quaternion.AngleAxis(-halfFOV, visionTransform.up);
            var rightRayRotation1 = Quaternion.AngleAxis(halfFOV, visionTransform.up);

            var leftRayDirection1 = leftRayRotation1 * visionTransform.forward;
            var rightRayDirection1 = rightRayRotation1 * visionTransform.forward;

            var npcSight = charTransform.position + visionPosition;
        ...
        public void Validate()
        {
...
#if UNITY_EDITOR
                if (EditorUtility.DisplayDialog("Min Sight Level is too large",
                        "Min Sight Level can't be as big as the Y axis of Vision Position. It'll automatically be decremented.",
                        "Ok"))
                {
                }
#endif
            ...
            if (maxSightLevel < visionPosition.y) maxSightLevel = visionPosition.y;
        }
        ...
        public int GetAlertTagIndex(string alertTag)
        {
            for (var i = 0; i < alertTags.Length; i++)
            {
                ...
                if (alertTags[i].alertTag.Length <= 0) continue;

                // check if alert tag equals the paramater
                if (alertTags[i].alertTag != alertTag) continue;
        ...
        {
            for (var i = 0; i < hostileTags.Length; i++)
            for (var x = 0; x < alertTags.Length; x++)
                if (hostileTags[i].Length > 0 && hostileTags[i] == alertTags[x].alertTag)
                {
#if UNITY_EDITOR
                    if (EditorUtility.DisplayDialog("Same tag in Hostile and Alert",
                            "You can't have the same tag name in both Hostile and Alert. The tag name in Alert Tags will be removed when out of focus or you can continue typing by double clicking the text.",
                            "Ok"))
                    {
                    }
#endif
