...
public class BlazeAIDistanceCulling : MonoBehaviour
{
    ...
    [Range(0, 30)]
    [Tooltip(
        "Run the cycle every set frames. The bigger the number, the better it is for performance but less accurate.")]
    public int cycleFrames = 7;

    [Range(0, 10)]
    [Tooltip(
        "This sets how many frames to rest before moving to the next calculation in the loop. This helps spread the distance culling calculations across several frames which improves performance.")]
    public int restFrames = 2;

    [Tooltip(
        "If set to true, the culling will be disabling Blaze only and playing the idle animation set in the [Anim To Play On Cull] property in Blaze inspector. When within range again, Blaze will re-enable.")]
    ...
    private readonly List<BlazeAI> agentsList = new();
    private int framesPassed;

    private int restFramesPassed;
    ...
    private void Awake()
    {
        ...
        if (cameraOrPlayer == null)
            Debug.LogWarning(
                "No camera has been set in the camera property in the Blaze AI Distance Culling component.");

        if (restFrames < 0) restFrames = 0;
    ...
    public virtual void Update()
    {
        ...
        if (!isLooping) StartCoroutine("RunCulling");
    }
    ...
    public virtual IEnumerator RunCulling()
    {
        ...
        for (var i = 0; i < max; i++)
        {
            ...
            if (i > agentsList.Count - 1) break;

            var blaze = agentsList[i];
            if (blaze == null)
            {
            ...
            var agentDistance = (blaze.transform.position - cameraOrPlayer.position).sqrMagnitude;
            ...
            if (agentDistance > distanceToCull * distanceToCull)
            {
                if (disableBlazeOnly)
                {
                    ...
                    if (blaze.state != BlazeAI.State.death) PlayCullAnim(blaze);
    ...
    {
        if (agentsList.Contains(agent)) return true;

        return false;
    }

    // play the cull animation
    private void PlayCullAnim(BlazeAI blaze)
    {
        if (blaze.animToPlayOnCull.Length > 0 && blaze.animToPlayOnCull != null)
            blaze.animManager.Play(blaze.animToPlayOnCull);
    }
