...
namespace Animmal.NotificationSystem
{
    ...
    public class ItemDisplayDataTextAndSprites : ItemDisplayDataBase
    {
        ...
        {
            for (int i = 0; i < _Data.Textures.Count; i++)
            {
                if (i < RawImages.Count)
                {
                    RawImages[i].texture = _Data.Textures[i];
                }
                else if (RawImages.Count > 0)
                {
                    Debug.LogError("Ultimate Notification System: NotificationItem - " + gameObject.name + " does not have enough Raw Images to display all Notification Data textures");
                }
            }
