...
namespace MHLab.Patch.Core.Client.IO
{
    public static class DownloaderHelper
    {
        ...
        public static string UrlEncode(string remoteUrl)
        {
            var builder = new UriBuilder(remoteUrl);

            var splitUrl = builder.Path.Split('/');

            for (var i = 0; i < splitUrl.Length; i++)
            {
                if (splitUrl[i].Contains("+"))
                    splitUrl[i] = splitUrl[i].Replace("+", "%2B");
            }

            return $"{builder.Scheme}://{builder.Host}:{builder.Port}{string.Join("/", splitUrl)}";
        }

        public static HttpWebRequest GetRequest(string remoteUrl, IWebProxy proxy, ICredentials credentials)
        {
        ...
        public static HttpWebRequest GetRequest(string remoteUrl, long offsetStart, long offsetEnd, IWebProxy proxy,
