...
//Also you need to choose Firepoint, targets > 1, Aim image from canvas and 2 target markers and camera.
...
public class HS_MovementInput : MonoBehaviour
{
    ...
    [Space] [Header("Animation Smoothing")] [Range(0, 1f)]
    ...
    [Space] [Header("Effects")] public GameObject TargetMarker;
    ...
    [Space] [Header("Canvas")] public GameObject[] ultIcons;
    ...
    [Space] [Header("Camera Shaker script")]
    ...
    [Space] [Header("Sound effects")] private AudioSource soundComponent; //Play audio from Prefabs
    ...
    private void Start()
    {
        ...
        if (Prefabs[8].GetComponent<AudioSource>()) soundComponent = Prefabs[8].GetComponent<AudioSource>();
    }
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButtonDown(1) && casting) casting = false;
        ...
        if (Input.GetKeyDown("3")) StartCoroutine(PreCast(2));
        if (Input.GetKeyDown("4")) StartCoroutine(PreCast(3));
        if (Input.GetKeyDown("z")) StartCoroutine(FrontAttack(4));
        if (Input.GetKeyDown("x")) StartCoroutine(FrontAttack(5));
        ...
        //Disable moving and skills if already using one
        if (!canMove)
            return;

        if (Input.GetMouseButton(0) && aim.enabled && activeTarger)
        {
            if (rotateState == false) StartCoroutine(RotateToTarget(fireRate, target.position));
            secondLayerWeight = Mathf.Lerp(secondLayerWeight, 1, Time.deltaTime * 10);
            if (fireCountdown <= 0f)
            {
                var projectile = Instantiate(PrefabsCast[8], FirePoint.position, FirePoint.rotation);
                projectile.GetComponent<HS_TargetProjectile>().UpdateTarget(target, uiOffset);
        ...
        if (Input.GetMouseButtonDown(1) && aim.enabled && activeTarger)
        {
            ...
            var buff = Instantiate(PrefabsCast[9], target.position, target.rotation);
            buff.transform.parent = target;
            var buffPS = buff.GetComponent<ParticleSystem>();
    ...
    public IEnumerator FastPlay(int EffectNumber)
    {
        ...
        Effect.Play();
        var parentPlace = PrefabsCast[EffectNumber].transform.parent;
    ...
    public IEnumerator Ult(int EffectNumber, float enableTime, float dissableTime, Vector3 pivotPosition,
        Quaternion pivotRotation, bool ChangePos)
    {
        ...
        while (true)
        {
            ...
            if (UltimatePrefab[EffectNumber] && useUlt)
            {
                ...
                //Play effect
                var ultPS = UltimatePrefab[EffectNumber].GetComponent<ParticleSystem>();
                ultPS.Play();
                //Shake camera
    ...
    private void UserInterface()
    {
        ...
        var CornerDistance = screenPos - screenCenter;
        var absCornerDistance = new Vector3(Mathf.Abs(CornerDistance.x), Mathf.Abs(CornerDistance.y),
        ...
        if (absCornerDistance.x < screenCenter.x / 3 && absCornerDistance.y < screenCenter.y / 3 && screenPos.x > 0 &&
            screenPos.y > 0 && screenPos.z > 0 //If target is in the middle of the screen
            && !Physics.Linecast(transform.position + (Vector3)uiOffset, target.position + (Vector3)uiOffset * 2,
                collidingLayer)) //If player can see the target
        {
        ...
        if (TargetMarker2 && casting == false)
        {
            ...
            while (true)
            {
                ...
                if (Input.GetMouseButtonDown(0) && casting == false)
                {
                    ...
                    if (rotateState == false) StartCoroutine(RotateToTarget(1, vecPos));
                    ...
                    foreach (var component in Prefabs[EffectNumber].GetComponentsInChildren<HS_FrontAttack>())
                        component.PrepareAttack(vecPos);
                    //Use ult after main skill
                    if (UltimatePrefab[EffectNumber] != null)
                    {
                        if (EffectNumber == 7)
                            StartCoroutine(Ult(EffectNumber, 0f, 1.5f, new Vector3(1, 1, 1),
                                Quaternion.LookRotation(forwardCamera), true));
                        else
                            StartCoroutine(Ult(EffectNumber, 0f, 1.5f, new Vector3(0, 0, 0),
                                Quaternion.LookRotation(forwardCamera), false));
                    }
    ...
    public IEnumerator PreCast(int EffectNumber)
    {
        if (PrefabsCast[EffectNumber] && TargetMarker)
            ...
            while (true)
            {
                ...
                forwardCamera.y = 0.0f;
                RaycastHit hit;
                var ray = new Ray(Camera.main.transform.position + new Vector3(0, 2, 0), Camera.main.transform.forward);
                if (Physics.Raycast(ray, out hit, Mathf.Infinity, collidingLayer))
                {
                    TargetMarker.transform.position = hit.point;
                    TargetMarker.transform.rotation = Quaternion.FromToRotation(Vector3.up, hit.normal) *
                                                      Quaternion.LookRotation(forwardCamera);
                }
                else
                {
                    aim.enabled = true;
                ...
                if (Input.GetMouseButtonDown(0) && casting == false)
                {
                    ...
                    if (rotateState == false) StartCoroutine(RotateToTarget(1, hit.point));
                    casting = true;
                    ...
                    //Use ult after main skill
                    if (UltimatePrefab[EffectNumber] != null)
                        StartCoroutine(Ult(EffectNumber, 0.5f, castingTime[EffectNumber], hit.point,
                            Quaternion.LookRotation(forwardCamera), true));
                    yield break;
                }
        ...
        while (true)
            if (casting)
            {
                if (castingTime[EffectNumber] == 0)
                {
                    //Play PrefabCast VFX
                    currEffect.Play();
                    if (soundComponentCast) CastSoundPlay();
        ...
        while (true)
        {
            if (casting)
            {
                if (castingTime[EffectNumber] == 0)
                {
                    ...
                    Effect.Play();
                    //Play sound FX if exist
                    if (soundComponent) MainSoundPlay();
    ...
    private void PlayerMoveAndRotation()
    {
        ...
        if (blockRotationPlayer == false)
        {
            ...
            transform.rotation =
                Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(forward), desiredRotationSpeed);
            //Limit back speed
            if (InputZ < -0.5)
    ...
    public int targetIndex()
    {
        ...
        for (var i = 0; i < screenTargets.Count; i++)
