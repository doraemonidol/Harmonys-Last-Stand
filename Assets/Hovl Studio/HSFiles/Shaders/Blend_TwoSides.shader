...
Shader "Hovl/Particles/Blend_TwoSides"
{
    ...
    SubShader
    {
        Tags
        {
            "RenderType" = "TransparentCutout" "Queue" = "Transparent+0" "IsEmissive" = "true" "PreviewType"="Plane"
        }
        Cull Off
        ...
        void surf(Input i, inout SurfaceOutput o)
        {
            ...
            float4 temp_output_70_0 = (tex2D(_Mask, uv_Mask) * tex2D(
                _Noise, ((uvs_Noise).xy + (_Time.y * appendResult22) + uvs_Noise.w)) * ((_UseCustomData)
                         ? (uvs_Noise.z)
                         : (1.0)));
            float4 Noise156 = saturate(
                (float4(-1, -1, -1, -1) + (temp_output_70_0 - float4(0, 0, 0, 0)) * (float4(2, 2, 2, 2) -
                    float4(-1, -1, -1, -1)) / (float4(1, 1, 1, 1) - float4(0, 0, 0, 0))));
            ...
            float4 lerpResult91 = lerp(
                ((_UseFresnel)
                    ? (((_FrontFacesColor * (1.0 - fresnelNode95) * (
                        (_Usesmoothcorners) ? (Noise156) : (float4(1, 1, 1, 1)))) + (_FresnelEmission *
                        _FresnelColor * (
                            (_Usesmoothcorners) ? (saturate((fresnelNode95 + (1.0 - Noise156)))) : (temp_cast_2)))))
                    : (_FrontFacesColor)),
                ((_Usesmoothcorners)
            ? (((_UseBackFresnel)
                                                   ? (((_BackFacesColor * (1.0 - fresnelNode146) * Noise156) + (
                                                       _BackFresnelEmission * _BackFresnelColor *
                                                       saturate((fresnelNode146 + (1.0 - Noise156))))))
                                                   : (_BackFacesColor)))
            : (_BackFacesColor)), (1.0 + (sign(dotResult87) - -1.0) * (0.0 - 1.0) / (1.0 - -1.0)));
            ...
            o.Emission = ((_SeparateFresnel)
                              ? (((lerpResult91 + (_FresnelColor * tex2DNode105 * _SeparateEmission)) * _Emission * i.
                                  vertexColor))
                              : ((lerpResult91 * _Emission * i.vertexColor * tex2DNode105))).rgb;
            float4 temp_cast_4 = (i.vertexColor.a).xxxx;
            float4 temp_cast_5 = (1.0).xxxx;
            o.Alpha = ((_Sideopacity)
                           ? ((i.vertexColor.a * saturate(
                               ((temp_output_70_0 - temp_cast_5) + ((_UseCustomData) ? (uvs_Noise.z) : (1.0))))))
                           : (temp_cast_4)).r;
