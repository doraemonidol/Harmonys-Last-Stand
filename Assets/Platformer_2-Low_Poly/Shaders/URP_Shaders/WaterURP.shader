...
Shader "Water"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float voronoi109(float2 v, float time, inout float2 id, inout float2 mr, float smoothness,
                inout float2 smoothId)
            {
                float2 n = floor(v);
                float2 f = frac(v);
            ...
            VertexOutput VertexFunction(VertexInput v)
            {
                ...
                float screenDepth317 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH_LOD(ase_screenPosNorm.xy),
                         _ZBufferParams);
                float distanceDepth317 = abs(
                    (screenDepth317 - LinearEyeDepth(ase_screenPosNorm.z, _ZBufferParams)) / (5.0));
            ...
            ) : SV_Target
            {
                ...
                float2 panner322 = (_TimeParameters.x * (_RefractionSpeed * _RefractionDirection) + ((WorldPosition).xz
                    * _RefractionTillingNoise));
                ...
                float4 fetchOpaqueVal27 = float4(SHADERGRAPH_SAMPLE_SCENE_COLOR(
                    ( float3( (ase_screenPosNorm).xy , 0.0 ) + ( worldToTangentDir42_g1 * _RefractionStr ) ).xy), 1.0);
                float4 Refraction77 = fetchOpaqueVal27;
                float eyeDepth182 =
                    LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH(ase_screenPosNorm.xy), _ZBufferParams);
                ...
                float4 lerpResult282 = lerp(Refraction77, (lerpResult277 + fresnelNode262),
                                               saturate((temp_output_181_0 * _DepthMultiply)));
                float4 temp_cast_2 = (1.0).xxxx;
                float screenDepth247 = LinearEyeDepth(
                    SHADERGRAPH_SAMPLE_SCENE_DEPTH(ase_screenPosNorm.xy), _ZBufferParams);
                float distanceDepth247 = abs(
                    (screenDepth247 - LinearEyeDepth(ase_screenPosNorm.z, _ZBufferParams)) / (1.0));
                float simplePerlin2D245 = snoise((WorldPosition).xz * _ShoreWaveScale);
                simplePerlin2D245 = simplePerlin2D245 * 0.5 + 0.5;
                float screenDepth249 = LinearEyeDepth(
                    SHADERGRAPH_SAMPLE_SCENE_DEPTH(ase_screenPosNorm.xy), _ZBufferParams);
                float distanceDepth249 = abs(
                    (screenDepth249 - LinearEyeDepth(ase_screenPosNorm.z, _ZBufferParams)) / (1.0));
                float clampResult248 = clamp(
                    (sin(((1.0 - (distanceDepth247 / _ShoreWaveWidth)) + (_TimeParameters.x * _ShoreWaveSpeed))) +
                        simplePerlin2D245), 0.0, (1.0 - (distanceDepth249 / _ShoreWidth)));
                float screenDepth97 = LinearEyeDepth(
                    SHADERGRAPH_SAMPLE_SCENE_DEPTH(ase_screenPosNorm.xy), _ZBufferParams);
                float distanceDepth97 = abs(
                    (screenDepth97 - LinearEyeDepth(ase_screenPosNorm.z, _ZBufferParams)) / (_FoamWidth));
                ...
                float simplePerlin2D100 = snoise(
                    (temp_output_104_0 + (simplePerlin2D290 * _FoamDistortionAmount)) * _FoamScale);
                simplePerlin2D100 = simplePerlin2D100 * 0.5 + 0.5;
                float4 lerpResult265 = lerp(lerpResult282, temp_cast_2,
                                                  saturate(max(step((1.0 - clampResult248), 0.9),
                                    saturate(step(distanceDepth97, simplePerlin2D100)))));
                ...
                float2 uv109 = 0;
                float voroi109 = voronoi109(coords109, time109, id109, uv109, 0, voronoiSmoothId109);
                float4 lerpResult307 = lerp(lerpResult265, temp_cast_4,
                                            saturate(((pow(voroi130, _WaveSmallPower) + pow(
                                                voroi109, _WaveBigPower)) * _WavesMultiply)));
                ...
                float3 localPerturbNormal107_g6 = PerturbNormal107_g6(surf_pos107_g6, surf_norm107_g6, height107_g6,
                    scale107_g6);
                float3 worldToTangentDir42_g6 = mul(ase_worldToTangent, localPerturbNormal107_g6);

                float screenDepth81 = LinearEyeDepth(
                    SHADERGRAPH_SAMPLE_SCENE_DEPTH(ase_screenPosNorm.xy), _ZBufferParams);
                float distanceDepth81 = abs(
                    (screenDepth81 - LinearEyeDepth(ase_screenPosNorm.z, _ZBufferParams)) / (0.2));
        ...
        Pass
        {
            ...
            half4 frag(VertexOutput IN) : SV_TARGET
            {
                ...
                float4 lerpResult307 = lerp(lerpResult265, temp_cast_4,
                                            saturate(
                                                ((pow(voroi130, _WaveSmallPower) + pow(voroi109, _WaveBigPower)) *
                                                    _WavesMultiply)));
        ...
        Pass
        {
            ...
            TessellationFactors TessellationFunction(InputPatch<VertexControl, 3> v)
            {
                ...
                tf = EdgeLengthBasedTessCull(v[0].vertex, v[1].vertex, v[2].vertex, edgeLength, tessMaxDisp,
                    GetObjectToWorldMatrix(), _WorldSpaceCameraPos,
                    _ScreenParams, unity_CameraWorldClipPlanes);
                #endif
                o.edge[0] = tf.x;
                o.edge[1] = tf.y;
                o.edge[2] = tf.z;
