...
Shader "Water"
{
    ...
    SubShader
    {
        ...
        void vertexDataFunc(inout appdata_full v)
        {
            ...
            float screenDepth317 = LinearEyeDepth(
                SAMPLE_DEPTH_TEXTURE_LOD(_CameraDepthTexture, float4( ase_screenPosNorm.xy, 0, 0 )));
        ...
        void surf(Input i, inout SurfaceOutputStandard o)
        {
            ...
            float2 panner322 = (_Time.y * (_RefractionSpeed * _RefractionDirection) + ((ase_worldPos).xz *
            ...
            float4 screenColor27 = UNITY_SAMPLE_SCREENSPACE_TEXTURE(_GrabTexture,
                                          ( float3( (ase_screenPosNorm).xy , 0.0 ) + ( worldToTangentDir42_g1 *
                                              _RefractionStr
                                          ) ).xy);
            ...
            float clampResult248 = clamp(
                (sin(((1.0 - (distanceDepth247 / _ShoreWaveWidth)) + (_Time.y * _ShoreWaveSpeed))) + simplePerlin2D245),
                0.0, (1.0 - (distanceDepth249 / _ShoreWidth)));
