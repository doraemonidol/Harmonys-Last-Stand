...
Shader "GAP/AdditiveDistortionScroll"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            uniform sampler2D _MainTexutre;
            uniform float4 _MainTexutre_ST;
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR
            {
                ...
                float sceneZ = max(
                    0, LinearEyeDepth(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(i.projPos)))) -
                    _ProjectionParams.g);
                ...
                float2 node_2493 = lerp(i.uv0, lerp(i.uv0, float2(_Distortion_var.r, _Distortion_var.r), _NoiseAmount),
                    _DistortionMask.r);
                float2 node_902 = ((node_9939.g * float2(_MainTextureUSpeed, _MainTextureVSpeed)) + lerp(
                    i.uv0, node_2493, _DistortMainTexture));
                ...
                float3 emissive = (((_DistortionMask_copy.rgb * _MainTexutre_var.rgb) * i.vertexColor.rgb * (
                    _ColorMultiplier * _TintColor.rgb * _ColorRamp_var.rgb) * 2.0 * (_MainTexutre_var.a *
                    pow(_Gradient_var.r, _GradientPower) * clamp(isFrontFace, _DoubleSided, 1.0))) * saturate(
                    (sceneZ - partZ) / _EdgeSoftness));
