...
public class SpawnProjectilesScript : MonoBehaviour
{
    ...
    private readonly List<Camera> camerasList = new();
    ...
    private void Start()
    {
        ...
        {
            for (var i = 0; i < cameras.transform.childCount; i++)
        ...
        if (useTarget && target != null)
        {
            ...
            if (!collider) target.AddComponent<BoxCollider>();
        }
    }
        ...
        if (firePoint != null)
        {
            ...
            else
            {
                if (target != null)
                {
                    ...
                    rotateToMouse.RotateTo(vfx, target.transform.position);
                }
                else
                {
                    Destroy(vfx);
        ...
        if (camerasList.Count > 0)
        {
            if (!camerasList[0].orthographic)
            {
                if (camerasList[0].fieldOfView < 101)
                    for (var i = 0; i < camerasList.Count; i++)
