...
namespace GAP_ParticleSystemController
{
    ...
    public class SerializableAnimationCurve
    {
        ...
        {
            var keys_ = new SerializableKeyFrames[animCurve.keys.Length];
            for (var i = 0; i < animCurve.length; i++) keys_[i] = new SerializableKeyFrames(animCurve.keys[i]);
            keys = keys_;
        ...
        {
            var animCurv = new AnimationCurve();
            animCurv.keys = new Keyframe[keys.Length];
            for (var i = 0; i < keys.Length; i++) animCurv.keys[i] = keys[i].GetKeyFrames();
    ...
    public class SerializableAlphaKeys
    {
        ...
        {
            var alpha_ = new float[gradAlphaKeys.Length];
            var time_ = new float[gradAlphaKeys.Length];
            for (var i = 0; i < gradAlphaKeys.Length; i++)
            {
        ...
        {
            var gak = new GradientAlphaKey[alpha.Length];
    ...
    public class SerializableColorKeys
    {
        ...
        {
            var color_ = new SerializableColor[gradColorKeys.Length];
            var time_ = new float[gradColorKeys.Length];
            for (var i = 0; i < gradColorKeys.Length; i++)
            {
        ...
        {
            var gck = new GradientColorKey[color.Length];
    ...
    public class SerializablePSGradientMode
    {
        ...
        {
            var psGradientMode = new ParticleSystemGradientMode();
            switch (mode)
            {
                case "Color":
                {
    ...
    public class SerializableGradientMode
    {
        ...
        {
            var gradientMode = new GradientMode();
            switch (mode)
            {
                case "Blend":
                {
    ...
    public class SerializablePSCurveMode
    {
        ...
        {
            var psCurveMode = new ParticleSystemCurveMode();
            switch (mode)
            {
                case "Constant":
                {
